...
def calc_reversal_science(row, spot, max_oi, max_vol, max_gamma):
    ...
    # Base floor of 55%, scaled up by structural strength
...
# Safety Check: Only run if data is NOT None
if spot is not None and calls is not None and not calls.empty:
    ...
    # Mid-Range (Unique strikes closest to price)
    ...
    st.error("⚠️ Connection Error. The data source (Massive or Yahoo) is currently unavailable. Retrying automatically...")
